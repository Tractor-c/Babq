#ifndef BABQ_RING_SPMC_H
#define BABQ_RING_SPMC_H
...
namespace babq
{
    ...
    private:
        ...
        static void reset_cursor_heavy(std::atomic<uint64_t> &cursor, uint64_t new_version, uint64_t init_local);
    ...
    // Reset a block cursor to a new version with init value
    inline void SharedRingBuffer::reset_cursor_heavy(std::atomic<uint64_t> &cursor, uint64_t new_version, uint64_t init_local)
    {
    ...
    inline DeqStatus SharedRingBuffer::dequeue(Batch &out)
    {
        ...
        reset_cursor_heavy(next_blk.consumed, consumer_version + 1, 0);
        reset_cursor_heavy(next_blk.reserved, consumer_version + 1, 0);
